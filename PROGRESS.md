...
| **P10** | Real Compare engine | ✅ done (2026-05-18) | Client state in localStorage (`pp.compare`, max 4). Cross-tab sync. Live tray (`<CompareTray>`) + add/remove buttons (`<AddToCompareButton>`) on every detail page. |
| **P10.2** | Compare ALL documented attributes (not a thin slice) | ✅ done (2026-05-18) | Rebuilt `<CompareGrid>` to mirror the detail-page attribute-card structure. Pens: 8 grouped sections (Identity, Nib, Ink Delivery, Body, Dimensions, Performance, Heritage & Service, Pricing) — ~60 rows. Papers: 8 groups (Identity, Substance, Surface, Behaviour, Dry Time by Nib, Appearance, Format, Pricing) — ~45 rows. Inks: 7 groups (Identity, Colour & Hue, Chemistry, Behaviour, Dry Time, Pairing, Pricing) — ~50 rows. Progressive disclosure: row hidden if every entity is null on it; group hidden if every row is hidden. Match-row tinting where every visible value agrees. Mixed-kind selections fall back to identity-only rows. |
...
| D4 | Facet RPC functions | ✅ done (2026-05-18) | `<FilterableArchive>` client component with live chip counts, AND-across-groups + OR-within. 7/5/5/3 facet groups across pens/papers/inks/pairings. |
| D8 | Sommelier picker | ✅ done (2026-05-18) | `pair_match_by_axes(wet,tooth,sheen,heritage,occasion,lim)` RPC iterates every pen×paper combo, computes axes via `fn_score_pair`, Euclidean-distance ranks against the target vector. `<SommelierResults>` client component reads slider DOM, debounces 220ms, calls RPC, renders 5 ranked results in the prototype's `.picker-results` aside. Verified: defaults give Vanishing Point/3776/88 (mid-range); wet=95 promotes Custom 823 (the wettest pen) to top with affinity 82. |
